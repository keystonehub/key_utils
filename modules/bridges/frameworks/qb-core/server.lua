        ...
        if GetResourceState('ox_inventory') == 'started' then
            ...
            for _, item in ipairs(options.items) do
                if item.action == 'add' then
                    ...
                    TriggerClientEvent('qb-inventory:client:ItemBox', _src, core.Shared.Items[item.item_id], 'add', item.quantity)
                elseif item.action == 'remove' then
                    player.Functions.RemoveItem(item.item_id, item.quantity)
                    TriggerClientEvent('qb-inventory:client:ItemBox', _src, core.Shared.Items[item.item_id], 'remove', item.quantity)
                end
            end
        end
    end
