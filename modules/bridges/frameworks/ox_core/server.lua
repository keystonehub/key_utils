if config.framework ~= 'ox_core' or GetResourceState('ox_core') ~= 'started' then return end
...
local function player_has_job(_src, job_names)
    ...
    if tables_module.contains(job_names, player_jobs) and player.hasGroup(job_names) then
        job_found = true
