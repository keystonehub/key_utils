    ...
    local function proceed()
        for _, item in ipairs(options.items) do
            if item.action == 'add' then
                exports.ox_inventory:AddItem(source, item.item_id, item.quantity, item.data)exports.ox_inventory:AddItem(source, item.item_id, item.quantity, item.data)
            elseif item.action == 'remove' then
                exports.ox_inventory:RemoveItem(source, item.item_id, item.quantity, item.data)
            end
        end
    end
