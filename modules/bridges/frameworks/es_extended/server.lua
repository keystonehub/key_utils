if config.framework ~= 'es_extended' or GetResourceState('es_extended') ~= 'started' then return end
        ...
        if GetResourceState('ox_inventory') == 'started' then
            for _, item in ipairs(options.items) do
                if item.action == 'add' then
                    exports.ox_inventory:AddItem(_src, item.item_id, item.amount, item.data)exports.ox_inventory:AddItem(_src, item.item_id, item.amount, item.data)
            ...
            for _, item in ipairs(options.items) do
                if item.action == 'add' then
                    player.addInventoryItem(item.item_id, item.amount)
                elseif item.action == 'remove' then
                    player.removeInventoryItem(item.item_id, item.amount)
                end
            end
        end
    end
