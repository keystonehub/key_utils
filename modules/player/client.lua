...
local requests_module = utils.get_module('requests')
...
local function get_distance_to_entity(player_ped, entity)
    local player_coords = GetEntityCoords(player_ped)
...
local function play_animation(player_ped, options, callback)
    ...
    local progress_handlers = {
        circle = function(message)
            utils.show_circle({ message = message or 'Processing...', duration = duration / 1000 })
        end,
        bar = function(message, icon)
            utils.show_progressbar({ header = message or 'Working...', icon = icon or 'fa-solid fa-user', duration = duration })
        end,
    }
