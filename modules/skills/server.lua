...
local skills_list = utils.get_shared_data('skills')
...
local function insert_new_skill(_src, skill_name, skill_data)
    local columns, values, params = framework.get_insert_params(_src, 'skills', skill_name, skill_data)
...
local function modify_skill(_src, skill_name, value, operation)
    ...
    if not all_skill_data[skill_name] then
        ...
        local default_skill_data = {
            level = skills_list[skill_name].level,
            current_xp = skills_list[skill_name].start_xp,
            required_xp = calculate_required_xp(skills_list[skill_name].level, skills_list[skill_name].first_level_xp, skills_list[skill_name].growth_factor),
            first_level_xp = skills_list[skill_name].first_level_xp,
            growth_factor = skills_list[skill_name].growth_factor,
            max_level = skills_list[skill_name].max_level
        }
...
exports('modify_skill', modify_skill)
skills.modify_skill = modify_skill
...
callbacks.register('fivem_utils:sv:get_skills', fetch_player_skills)
...
callbacks.register('fivem_utils:sv:get_skill', fetch_skill)
