...
if ENV.IS_SERVER then
    ...
    RegisterServerEvent("key_utils:sv:trigger_callback")
    AddEventHandler("key_utils:sv:trigger_callback", function(name, data, client_cb_id)
        local source = source
        ...
        if callback then
            ...
            callback(source, data, function(...)
                TriggerClientEvent("key_utils:cl:client_callback", source, client_cb_id, server_cb_id, ...)
            end)
        else
    ...
    RegisterNetEvent("key_utils:cl:client_callback")
    AddEventHandler("key_utils:cl:client_callback", function(client_cb_id, server_cb_id, ...)
