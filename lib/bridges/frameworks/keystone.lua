if ENV.FRAMEWORK ~= "keystone" then return end

--- @section Modules

local TABLES <const> = require("modules.tables")
...
if ENV.IS_SERVER then
    ...
    local function add_item(source, item_id, amount, data)
        ...
        exports.keystone:add_item(source, item_id, amount, data)
    end
    ...
    local function remove_item(source, item_id, amount)
        ...
        exports.keystone:remove_item(source, item_id, amount)
    end
    ...
    local function add_balance(source, balance_type, amount, sender, note)
        exports.keystone:add_money(source, balance_type, amount, sender or nil, note or nil)
    end
    ...
    --- @param note string|nil: Optional note for the transaction.
    local function remove_balance(source, balance_type, amount, recipient, note)
        exports.keystone:remove_money(source, balance_type, amount, recipient or nil, note or nil)
    end
    ...
    local function get_data(key)
        local player_data = exports.keystone:get_player_data(key)
        return player_data
    end
