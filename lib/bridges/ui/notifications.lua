...
if ENV.IS_SERVER then
    ...
    local function notify(source, options)
        ...
        TriggerClientEvent("key_utils:cl:send_notification", source, options)
    end
    ...
    local HANDLERS <const> = {
        default = function(options)
            TriggerEvent("key_utils:cl:notify", { type = options.type, header = options.header, message = options.message, duration = options.duration })
        end,
